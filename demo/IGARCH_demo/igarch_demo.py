# -*- coding: utf-8 -*-
"""
IGARCH 波动率建模 —— 一条龙完整流程 (均值用 ARMA, 波动用 IGARCH(1,1)-t)

  IGARCH 是 GARCH 的"边界特例"(本 demo 的主角):
    GARCH(1,1):   sigma_t^2 = omega + alpha a_{t-1}^2 + beta sigma_{t-1}^2,  持续度 alpha+beta < 1
    IGARCH(1,1):  alpha + beta = 1  (Integrated GARCH, 方差有"单位根")
                  -> 持续度恰为 1: 一次波动冲击"永不消散"(半衰期 = 无穷)
                  -> 无有限无条件方差 omega/(1-alpha-beta) = omega/0 = 无穷
                  -> 多步波动预测"不回复": sigma^2(l) = sigma^2(1) + (l-1)*omega 线性外推, 不收敛
    著名特例: omega=0 时即 RiskMetrics/EWMA (sigma_t^2 = (1-lam)a_{t-1}^2 + lam sigma_{t-1}^2).

  与 GARCH 的对照 (一句话): GARCH 的波动"有记性但会忘", IGARCH 的波动"过目不忘".
    实务里 alpha+beta 估出来 >= 0.99 就该警惕已落入 IGARCH 领域, 别再套"均值回复"的预测.
  ...
  六步法 (下游一律用"定出来的阶", 不写死):
    ...
    第3步 定阶(p,q) : a_t^2 二维 AIC/BIC 网格 (与 GARCH 同, 常落 (1,1))
    第4步 估计参数  : eps_t ~ Student-t, MLE 估 omega/alpha/beta/nu; 重点看"持续度是否顶到 1"
    第5步 模型检验  : 标准化残差三查 + 持续度判定(≈1 -> IGARCH, 非协方差平稳)
    第6步 使用模型  : ARMA 点预测 + IGARCH 波动率"线性外推不回复" -> 动态区间 + VaR

  工具说明: Python 的 arch 包均值只支持 AR(无 MA), 故 ARMA 走"两步法":
    statsmodels 建 ARMA 取残差 a_t -> arch 对 a_t 建 GARCH(mean='Zero'); 持续度顶到 1 即 IGARCH.
"""
import sys, warnings
...
#   均值真值 = ARMA(1,1); 波动真值 = IGARCH(1,1)-t (alpha+beta=1.0, 积分/单位根).
# ====================================================================
...
# ====================================================================
# 步骤3  定阶 (p,q): a_t^2 二维 AIC/BIC 网格 (与 GARCH 同)
# ====================================================================
print(SEP); print("步骤3  定阶 (p,q) (波动阶数, 二维网格)")
PMAX = QMAX = 2
ggrid_bic, ggrid_aic = {}, {}
...
for p in range(1, PMAX + 1):
    ...
    print(f"     p={p} {cells}")
print(f"  AIC 选 (p,q)={aic_order} | BIC 选 (p,q)={best_gorder}  >>> 最终定阶 (1,1) 形 (以BIC为准)")

fig, axx = plt.subplots(1, 2, figsize=(12, 4.5))
plot_acf(a2, lags=25, ax=axx[0], zero=False)
axx[0].set_title("ACF of a_t^2 (very slow decay -> persistence ~ 1)")
...
# ====================================================================
# 步骤4  估计参数: GARCH(p,q) on a_t, t 分布, MLE; 重点看持续度是否顶到 1
# ====================================================================
...
print(f"  >>> 持续度 alpha+beta = {persist:.4f} (真值 {persist_true:.2f})")
print(f"      {'>= 0.99: 已落入 IGARCH 领域 (方差有单位根, 非协方差平稳) [IGARCH]' if is_igarch else '< 1: 普通 GARCH'}")
print(f"  无条件波动率 = {'无穷 (IGARCH 无有限无条件方差!)' if not np.isfinite(uncond) else f'{uncond:.3f}'}")
print(f"  波动冲击半衰期 = {'无穷 (冲击永不消散)' if not np.isfinite(halflife) else f'{halflife:.1f} 天'}")
print(f"  logL={res.loglikelihood:.1f}, AIC={res.aic:.1f}, BIC={res.bic:.1f}")

# ====================================================================
# 步骤5  模型检验: 标准化残差三查 + 持续度判定
# ====================================================================
print(SEP); print("步骤5  模型检验 (标准化残差三查 + 持续度判定)")
...
#     -> sigma^2(l) = sigma^2(1) + (l-1)*omega: 线性外推, 永不收敛 (对比 GARCH 几何收敛).
# ====================================================================
print(SEP); print(f"步骤6  预测 (均值 {mean_desc} + 波动 IGARCH, {h} 步)")
...
print(f"  ★ 关键: 波动'不回复'到任何固定水平 -> 持续度={persist:.3f}≈1, 冲击永久, sigma 线性外推 (IGARCH 招牌)")
...
axx[0].set_title(f"Out-of-sample: mean {mean_desc} + vol IGARCH(persist={persist:.3f})")
...
# ====================================================================
print(SEP)
print(f"完成. 全流程: 平稳 -> 均值={mean_desc} -> ARCH效应 -> 定阶(1,1)形 "
      f"-> MLE(t) -> 持续度≈1判定 IGARCH -> 均值+'不回复'波动/VaR 预测")
